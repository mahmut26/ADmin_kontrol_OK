...
namespace Blog_Api.Controllers
{
    ...
    public class AdminController : ControllerBase
    {
        ...
        /// <summary>
        /// SQL SORGUSUNU DÜM DÜZ ATIYORU !! YENİ BU 27.08
        /// </summary>
       ...
       public IActionResult SQLQUERY(Link link)
        ...
        /// <summary>
        /// YEni yapıldı 27.08 12:26
        /// </summary>
        /// <param name="Sorgu"></param>
        /// <returns></returns>

        [HttpPost("MakaleDurumDegistir")]
        public IActionResult MakaleSQLQuery(Sorgu Sorgu)
        {
            string makale = Sorgu.Name;
            //string sorgu2 = link.sorgu2;
            string query = $@"UPDATE makales SET Goruldu = CASE WHEN Goruldu = 1 THEN 0 ELSE 1 END WHERE Baslik = @makale;";

            try
            {

                // ExecuteSqlRaw ile sorguyu çalıştırma
                _ctxt.Database.ExecuteSqlRaw(query, new SqlParameter("@makale", makale));
                return Ok("Query executed successfully.");
        ...
        [HttpGet("MakaleDurumlari")] //!!! Onay Yok Şu anda !!
        public async Task<IActionResult> MakaleDurumlari() //seç getir
        {

            var aa = _ctxt.makales.Select(x => new
            {
                x.Baslik,
                x.Goruldu
            ...
            List<DurumDeg> MakaleDurumları = new List<DurumDeg>(); //belkide bir daha kullanmayacağım viewlerden temizlemek lazım aslında

            foreach (var user in aa) //listeye ekleme sistemim
            {

                MakaleDurumları.Add(new DurumDeg
                {
                    Name = user.Baslik,
                    Degistir = user.Goruldu,
                    
                });
            }

            return Ok(MakaleDurumları); //döndür listeyi
        }
    }
