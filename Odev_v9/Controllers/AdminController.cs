...
namespace Odev_v9.Controllers
{
    public class AdminController : Controller
    {
        ...
        /// <summary>
        /// Bunu uğraştırdı biraz 27.08 de yapıldı !!!
        /// </summary>
        ...
        [HttpGet]
        public async Task<IActionResult> MakaleDurum() //api durumları getiriyoru bu hepsinde aynı kodda değişiklik yapıp çalışıyor. mantık aramaya gerek yok 
        {
            string token = HttpContext.Session.GetString("Token"); //tokeni aldım 
            var client = _httpClientFactory.CreateClient();
            client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token); //tokeni jwt (jwt nasıl bearer olabilir b yok jwt de)
            ...
            string baseUrl = "https://localhost:7052/api/";
            string metot = "Admin/MakaleDurumlari";
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    var linkInt2List = JsonConvert.DeserializeObject<List<DurumDeg>>(responseContent); //parçaladım 
        ...
        /// <returns></returns>
        [HttpGet]
        public async Task<IActionResult> UpdateMakaleStatus(string userName)
        {
            ...
            if (userName != null)
            {
                    ...
                    string baseUrl = "https://localhost:7052/api/";
                    string metot = "Admin/MakaleDurumDegistir";
                ...
                DurumDeg dd = new DurumDeg()
                {
                    Name = userName,
                };

                    var content = new StringContent(JsonConvert.SerializeObject(dd), Encoding.UTF8, "application/json"); //api ye gönderiyor
                    try
                    {
                        HttpResponseMessage response = await client.PostAsync(url, content);
